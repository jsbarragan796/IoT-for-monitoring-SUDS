...
router.post('/', async (req, res) => {
  ...
  const numberOfEvents = await EventLogic.numberOfEvents(filterData)
  ...
  try {
    if ( pageNumber && (pageNumber>numberOfPages || pageNumber<0) ) throw new Error("Pagina fuera de rango")
      ...
      const events = await EventLogic.findFinishedEventsFilter(firstEventPage,eventsInPage,filterData)
